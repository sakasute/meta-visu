...
class TreeChart {
  constructor(data, svgElement, config) {
    this.config = {
      ...
      childrenNames: config.childrenNames ? config.childrenNames : ['registerAdmins', 'registers', 'categories', 'samplings'],
    ...
    const treeLayout = d3.tree().size([treeHeight, treeWidth]).separation(() => 1); // NOTE: this seems to spread all the leaf nodes equally far apart

    const hierarchy = d3.hierarchy(data, d => this.findChildArr(d));

    this.treeData = treeLayout(hierarchy);

    this.svg = svgElement.append('g').attr('class', 'tree').attr('width', this.config.width).attr('height', this.config.height).attr('transform', `translate(${this.config.posX}, ${this.config.posY})`).append('g');
  ...
  drawNodeCircles(nodeGroup) {
    nodeGroup.append('circle').attr('class', 'tree__node-marker').attr('r', this.config.nodeSize);
  ...
  addNodeLabels(nodeGroup) {
    nodeGroup.filter(d => d.parent).append('text').attr('class', 'tree__node-label').attr('dy', d => this.constructor.calculateLabelPlacement(d)).attr('x', -13).attr('text-anchor', 'middle').text(d => d.data.name);
  ...
  addRootLabel(rootNode) {
    ...
    const rootLabel = fo.append('xhtml:p').attr('class', 'tree__html-label').html(d => d.data.name);
  ...
  updateNodes() {
    ...
    // .filter(d => d.depth > 0)
    .append('g').attr('class', 'tree__node').attr('transform', () => `translate(${this.sourceCoord.y}, ${this.sourceCoord.x})`);
    // .on('click', (d) => {
    //   this.clickNode(d);
    // });
    ...
    // .transition()
    // .duration(this.config.animationDuration)
    .attr('transform', `translate(${this.sourceCoord.y}, ${this.sourceCoord.x})`).remove();
  }
  ...
  updateLinks() {
    ...
    .insert('path', 'g').attr('class', 'tree__link').attr('d', () => {
    ...
    // exit
    linkSelection.exit().transition().duration(750).attr('d', () => {
